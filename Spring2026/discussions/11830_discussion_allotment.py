...
MUST_HAVE_ROLES = ["Within-Week Connector", "Evaluation and Validity Auditor"]
GOOD_TO_HAVE_ROLES = [
...
ASSIGNMENTS_DIR = "11830/Spring2026/discussions"
...
def load_previous_assignments(discussion_number):
    ...
    file_path = os.path.join(
        ASSIGNMENTS_DIR, f"assignments_discussion_{discussion_number}.json"
    )
...
def read_students(csv_file):
    ...
    with open(csv_file, newline="") as f:
        ...
        for row in reader:
            students.append(f"{row['Preferred/First Name']} {row['Last Name']}")
    return students
...
#                         other_past_roles = set(
#                             previous_assignments.get(other_student, [])
#                         )
#                         other_role = assignments[other_student]
#                         # Swap if it improves role variety
#                         if (
#                             other_role not in past_roles
#                             and current_role not in other_past_roles
#                         ):
#                             assignments[student], assignments[other_student] = (
#                                 assignments[other_student],
#                                 assignments[student],
#                             )
#                             groups[i][j], groups[k][l] = (
#                                 groups[k][l],
#                                 groups[i][j],
#                             )  # Swap students in groups
#                             break
#     return assignments
...
#             available_roles = (
#                 MUST_HAVE_ROLES
#                 + GOOD_TO_HAVE_ROLES[: num_students - len(MUST_HAVE_ROLES)]
#             )
#         else:
...
#             past_roles = set(previous_assignments.get(student, []))
#             possible_roles = [
#                 role for role in available_roles if role not in past_roles
#             ]
            ...
            if current_role in past_roles:
                ...
                for k, other_group in enumerate(groups):
                    ...
                    for l, other_student in enumerate(other_group):
                        ...
                        other_role = assignments[other_student]
                        # Check if swap improves role variety AND doesn't create duplicates
                        if (
                            other_role not in past_roles
                            and current_role not in other_past_roles
                            and other_role
                            not in [assignments[s] for s in group if s != student]
                            and current_role
                            not in [
                                assignments[s]
                            ...
                            groups[i][j], groups[k][l] = groups[k][l], groups[i][j]
                            # The assignments already have the correct roles, no need to swap them
                            break
    return assignments
...
def assign_roles(groups, previous_assignments):
    ...
    for group in groups:
        ...
        # Create a copy of available_roles for this group
        remaining_roles = available_roles.copy()

        for student in group:
            past_roles = set(previous_assignments.get(student, []))
            possible_roles = [
                role for role in remaining_roles if role not in past_roles
            ]

            if possible_roles:
                role = random.choice(possible_roles)
            else:
                role = random.choice(remaining_roles)  # Allow repeats if necessary
...
def main(csv_file, discussion_number, num_groups):
    ...
    for i, group in enumerate(groups, 1):
        ...
        sorted_group = sorted(
            group, key=lambda x: ALL_ROLES.index(assignments[x])
...
if __name__ == "__main__":
    csv_file = "11830/Spring2026/discussions/11830_roster_260126.csv"  # Update
    discussion_number = 1  # Enter discussion number
    num_groups = 6  # Enter number of groups/papers selected
